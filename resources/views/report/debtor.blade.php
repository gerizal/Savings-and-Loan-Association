...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Daftar Pelunasan Debitur</h1>
            </div>
            <!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">Daftar Pelunasan Debitur</li>
                </ol>
            </div>
            <!-- /.col -->
        </div>
...
@push('script')
    <script>
        ...
        $(document).on('click', '.btn-print', function(event) {
            ...
            $.ajax({
                url: "{{ route('report.generate-report-debtor') }}",
                type: "post",
                data: params,
        ...
        function getData(){
            var employeeDT = $('#dataTable').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: "{!! route('report.debtor-datatable') !!}",
                    type: 'POST',
                    headers: {'X-CSRF-TOKEN': "{{ csrf_token() }}"}
                },
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', searchable: false, orderable: false},
                    { data: 'status_pelunasan', name: 'status_pelunasan', searchable: true, orderable: true },
                    { data: 'area_pelayanan', name: 'area_pelayanan', searchable: true, orderable: true },
                    { data: 'nopen', name: 'nopen', searchable: true, orderable: false },
                    { data: 'no_sk', name: 'no_sk', searchable: true, orderable: false },
                    { data: 'nama_pemohon', name: 'nama_pemohon', searchable: true, orderable: true },
                    { data: 'no_akad', name: 'no_akad', searchable: false, orderable: false },
                    { data: 'tanggal_akad', name: 'tanggal_akad', searchable: false, orderable: true },
                    { data: 'berkas_pelunasan', name: 'berkas_pelunasan', searchable: false, orderable: false },
                    { data: 'tanggal_pelunasan', name: 'tanggal_pelunasan', searchable: false, orderable: true },
                    { data: 'sumber_dana', name: 'sumber_dana', searchable: true, orderable: false },
                    { data: 'produk_pembiayaan', name: 'produk_pembiayaan', searchable: true, orderable: true },
        ...
        function reloadDataTable(){
            ...
            $('#dataTable').DataTable().ajax.url("{!! route('report.debtor-datatable') !!}?"+params.toString()).load();
        }
