...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Detail Pengajuan Slik</h1>
            </div>
            <!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{route('home')}}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{route('application.slik.index')}}">Pengajuan Slik</a></li>
                                    ...
                                    <div class="col-md-6">
                                        <div class="text-right">
                                            @if($data->status=='pending' || $data->status == 'queue')
                                                @if(check_access('pengajuan_slik','approve'))
...
<script>
    ...
    function approveReject(status, description = ''){
        var token = "{{ csrf_token() }}";
        $.ajax({
            url: "{{ route('application.slik.approve-reject',$data->id) }}",
            method: 'POST',
            data: {
                '_token': token,
                'status': status,
