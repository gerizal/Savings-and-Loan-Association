...
class ApprovalController extends Controller
{
    public function __construct(){
        $this->middleware('role.feature.check:approval_bank,view', ['only' => ['index']]);
        $this->middleware('role.feature.check:approval_bank,approve', ['only' => ['approveReject']]);
    }
    /**
    ...
    public function index()
    {
        ...
        return view('application.approval.index', compact('banks'));
    }
    ...
    public function show($id)
    {
        ...
        if($application){
            ...
            $data=[
                'data'   =>$application,
                'form' =>[
                    'url'       => route('application.approval.approve-reject',$application->id),
                    'method'    => 'POST',
                    'files'     => true,
                    'id'        =>'form-loan'
                ],
            ...
            return view('application.approval.view', $data);
        }else{
            return redirect('404');
        }
    }
    ...
    {
        $data = Approval::dataTable($request);
        return datatables()->of($data)
        ->addColumn('action', function($row){
            $action = '<div class="text-center">
                    <a href="'.route('application.approval.show', $row->application_id).'" class="btn btn-info btn-sm btn-md"><i class="fas fa-eye"></i> Detail</a>
                </div>';
            return $action;
        })
        ...
        ->addIndexColumn()
        ->rawColumns(['action','status','slik_status','verification_status','updated_at'])
        ->make(true);
    }
    ...
    public function approveReject(Request $request, $id)
    {
        ...
        try {
            ...
            if($data){
                ...
                $approval->save();
                $data->status=$approval->status == 'reject' ? 5:1;
                $data->approval_status = $request->status;
                $data->save();
            }
            DB::commit();
            return redirect()->route('application.approval.index')->withSuccess('Success');
        } catch (\Exceptions $e) {
            DB::rollback();
