...
class SlikController extends Controller
{
    public function __construct(){
        $this->middleware('role.feature.check:verifikasi_slik,view', ['only' => ['index']]);
        $this->middleware('role.feature.check:verifikasi_slik,approve', ['only' => ['approveReject']]);
    }
    /**
    ...
    public function index()
    {
        ...
        return view('application.slik.index', compact('banks'));
    }
    ...
    public function show($id)
    {
        ...
        if($application){
            ...
            $slik = Slik::whereApplicationId($id)->first();
            if($slik){
                $application->status = $slik->status;
                $application->checker_name = $slik->checker_name;
                $application->checked_by = $slik->checked_by;
                $application->checked_at = $slik->updated_at;
            }
            ...
            $data=[
                'data'   =>$application,
                'form' =>[
                    'url'       => route('application.slik.approve-reject',$application->id),
                    'method'    => 'POST',
                    'files'     => true,
                    'id'        =>'form-loan'
                ],
            ...
            return view('application.slik.view', $data);
        }else{
            return redirect('404');
        }
    }
    ...
    {
        $data = Slik::dataTable($request);
        return datatables()->of($data)
        ->addColumn('action', function($row){
            $action = '<div class="text-center">
                    <a href="'.route('application.slik.show', $row->application_id).'" class="btn btn-info btn-sm btn-md"><i class="fas fa-eye"></i> Detail</a>
                </div>';
            // $action .= '<div class="text-center">
    ...
    public function approveReject(Request $request, $id)
    {
        ...
        try {
            ...
            if($data){
                ...
                $slik->application_id       = $id;
                $slik->status               = $request->status;
                $slik->checked_by           = $user->id;
                $slik->checker_name         = $user->name;
                $slik->description          = $request->description;
                $slik->save();
                $data->slik_status = $request->status;
                $data->save();
            }
            DB::commit();
            return redirect()->route('application.slik.index')->withSuccess('Success');
        } catch (\Exceptions $e) {
            DB::rollback();
    ...
    public function delete($id)
    {
        try {
            ...
            if(check_access('pengajuan_slik','delete')  == true){
                ...
                return redirect()->route('application.slik.index');
            }else{
                return back();
            }
        } catch (\Exception $e) {
